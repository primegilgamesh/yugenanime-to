...
const HeroBanner = () => {
  ...
  const handleFavorite = () => {
    if (!isLoggedIn) {
      toast.error("Please login first to favorite");
      return;
    }
    setIsFavorited((v) => !v);
  };
        ...
        <div className="flex items-end gap-3 mb-2 md:mb-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="hidden md:flex items-center gap-3">
              <button
                onClick={() => handleAuthAction("add to list")}
                className="bg-primary text-primary-foreground font-semibold text-sm px-5 py-2 rounded-md hover:opacity-90 transition"
              >
                Add to List
              </button>
              <button
